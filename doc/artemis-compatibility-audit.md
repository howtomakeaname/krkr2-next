...
- 宿主使用原包副本：LOAD 菜单显示原逻辑槽位 `No.001-01`（诗樱）和 `No.001-05`（妃爱）及原日期/对白；点击 `No.001-01`、确认后从 `save0001.dat` 恢复 633 条图层指令及对应对白，再点击进入下一句。随后在独立测试目录保存 `No.001-12`，游戏分配 `save0003.dat`，保留原槽位和原下载目录不变。原 BOWG 加 5 个 BOWS 的解码回归通过。
...
- `takess` 捕获保留的场景 FBO；`savess` 使用 alpha 正确的双线性缩放写出指定尺寸 PNG，并原子替换文件；菜单后续重绘不改变已捕获图片。`lyc` 和 `e:file` 能从已有授权的 SaveDir 读取截图，绝对路径仍限制在该目录内。宿主真实 SAVE 页的新 `No.001-12` 显示妃爱场景的 `save0003.png`（192×109），不是菜单画面。未导入原版内嵌截图，也未穷举转场中捕获的原版时序；日期/编号和标题/正文挤压仍存在。
...
`libartemis.so` 与 `libartemis-compatible-v2.so` 含 E-mote 符号，其余几个被检查的版本未检出；兼容引擎现有模型数据与有限 C++ 场景渲染基础，尚无接入游戏脚本的完整播放器。天梨此次脸部错位是已修复的 PNG 换图尺寸问题，不能由此推广为 E-mote 已可用。
