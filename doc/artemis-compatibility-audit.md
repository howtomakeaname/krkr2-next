...
- `0f7f0cb`：同一图层重载图片时更新自然宽高、清除旧裁剪 UV，同时保留位置、锚点及父级变换。天梨 `a0054`（200×152）→ `a0051`（280×176）→ `a0002`（208×176）的 PNG 差分在宿主 GLES 路径复现了眼睛、嘴部缩小偏移；修复后尺寸与 PNG 元数据一致。该场景使用 PNG 差分，未经过 E-mote。
- `7538adc`：执行 `tweenset` 的同属性连续片段、loop 重复和 yoyo 回程，保留各片段时长与隐式起点。用合成测试检查 120/60/60 ms 点头全过程、回程中点、无限循环及取消。尚不代表完整 shader/动画语义已覆盖。
- Mate80 已安装含 `0f7f0cb` 的签名 HAP。实际课堂剧情截图中立绘及左下头像对齐；日志中 `a0002` 立绘差分为 `(526,183 208×176)`、头像差分为 `(103,539 120×100)`。动画后续提交尚未包含在这次设备包中。
- 新增游戏资源截图和反汇编记录仅保存在本地 `agent-artifacts/`；回归测试只使用合成图像与合成字体。视频、完整文字排版/逐字、按键及自动阅读、原版存档快照兼容仍需继续完成，不能据此宣称已与原版等效。
